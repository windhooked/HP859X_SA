...
> **Assessment & modernization blueprint:** [docs/ASSESSMENT_AND_MODERNIZATION.md](docs/ASSESSMENT_AND_MODERNIZATION.md) is an adversarially-verified (2026-06-14) audit of the whole RE/emulation state (subsystem grades, the MMIO contract surface, the timing-model verdict, the hardware-driver-layer inventory) plus a five-path modernization blueprint (original-FW-on-new-HW / native reimpl / Zynq-FPGA / swap-a-block / **minimal-M68K headless-RF-front-end**) and a ground-truth capture appendix. The recipe that generated it: [docs/ASSESSMENT_PROMPT.md](docs/ASSESSMENT_PROMPT.md). **Headline (VERIFIED): boots to the operating UI, POST clean, graticule renders, sweep buffer fills with a correct spectrum — but trace-line paint, front-panel input dispatch, and HP-IB query responses are all still control-flow / unknown-contract gated. The #1 cross-cutting modernization unknown is the A7-bus (0xFFF728/72A) register→DAC semantic map.**
...
**Musashi integration:** Musashi is compiled as a unity build (`musashi_core.c` includes all `.c` TUs). `m68kconf.h` is modified in-tree (vendored — we own it): `M68K_INSTRUCTION_HOOK M68K_OPT_SPECIFY_HANDLER`, `M68K_EMULATE_PMMU M68K_OPT_OFF`, **`M68K_EMULATE_ADDRESS_ERROR M68K_OPT_ON`** (`m68kconf.h:237` — the DLP-derail recovery; see the firmware-dump note). `RESET_CYCLES` drain and `SR` normalisation to `0x2700` are required after `m68k_pulse_reset()`. `m68k_execute(1)` loop for single-step (any M68K instruction takes ≥4 cycles; 1-cycle budget guarantees exactly one instruction). **Config-file footgun (RESOLVED 2026-07-12):** the build uses `m68kconf.h` because the cgo CFLAGS set **no `-DMUSASHI_CNF`** (`m68k.h:50` only includes the alternate config when that's defined). A stale orphan `pkg/emu/cpu/musashi/musashi_config.h` set `M68K_EMULATE_ADDRESS_ERROR OFF` and was **included by nothing** — it has now been **deleted** (`git rm`; build + tests re-verified green) so a future port that flips on `-DMUSASHI_CNF` can no longer silently disable address-error and re-open the DLP-derail failure mode. If you reintroduce an alternate config, keep `M68K_EMULATE_ADDRESS_ERROR ON`.
...
**Diagnostic commands** (`cmd/`): `dumprom [out.bin]` writes the canonical 1 MB Rev L image via `romloader.LoadDir` for external tools; `tracestall [cycles]` boots and disassembles the stall region + IRQ handlers; `disasm <from> <to>` disassembles a ROM range from the binary; `findref <word>` finds/disassembles references to a 16-bit value (e.g. an MMIO addr); `displayprobe [cycles]` histograms MMIO writes and dumps the ordered SCI stream; `keyprobe [cycles]` histograms MMIO reads + unmapped accesses; `renderframe [cycles] [out.png] [--synth]` renders the display to PNG (`--synth` populates CalNVRAM via `CalNVRAM.Synthesize()`); `caltrace [cycles] [top] [--faithful]` instruments `CalNVRAM.Trace` and dumps a histogram of which cal-NVRAM offsets the firmware reads/writes during boot (the tool that established Rev L has no boot-time cal gate byte); `tickprobe` boots, injects IRQ3, then runs the operating tick with fine-grained PC sampling — established that PC 0x18F42/0x18F3E (the deep-block key-flag bclr + parser jsr inside fcn.18568) are NEVER REACHED on Rev L under the 17.12.90-tuned `DriveOperatingTick` pre-arms; `jumptable` extracts the master + secondary dispatch tables and decodes the parser-name table (363 of 410 entries resolve after the secondary table at ROM 0x71E02 was identified — see [docs/DLP_RUNTIME.md](docs/DLP_RUNTIME.md)); `naturalkey [-boot N] [-run N] [-nokey] [-trace N]` is the **Path B** probe — boots naturally, injects a real front-panel key (no forced PC/gate bits), and either histograms post-boot PC regions or (`-trace`) single-steps the analog poll. It established that the 8593-strap boot froze in the `fcn.5E63C` analog `0x9A` status poll — **SUPERSEDED 2026-05-31**: the analog freeze is RESOLVED and the firmware now reaches the operating loop; the real (final) blocker is the `0xFFF300` bit-11 sweep-complete idle gate, now cracked. See the "★ 2026-05-31" note below and [docs/DRIVETICK_BLOCKER.md](docs/DRIVETICK_BLOCKER.md). `cmdref` decodes the **entire parser command table** (ROM `0x7C800..0x80000`) → the full GPIB/DLP command reference (see the note below).

**GPIB/DLP command reference + USTATE memory dump (2026-07-26).** `cmd/cmdref` extracts every parser command from the ROM (**802 mnemonics**; ~323 documented, ~299 `__`-prefixed DLP-internal, ~180 undocumented service/commissioning), resolves each handler (master table `0xC4+slot*6` / secondary DLP table `0x71E02`), auto-classifies it (DLP-source trampoline / constant stub / native), and merges a deep-RE behavior overlay → [docs/COMMAND_REFERENCE.md](docs/COMMAND_REFERENCE.md) + `docs/command_reference.json` (regenerate: `go run ./cmd/cmdref`). Behavior notes live in `docs/command_notes.json` (794 notes, 623 verified — 45-agent disassembly pass) with **`docs/command_notes_empirical.json` overriding** them (bench-verified on the real 8593E > disassembly inference). **Headline finding: `USTATE?` dumps user memory (downloaded DLPs, variables, saved traces) over GPIB as an HP `#A` block (bench: `#A`+~5244 B)** — `USTATE` is a DLP array variable (special-token class `type=0xc0`, array ID 12, beside the trace arrays `TRA=7..TRD=10`, shared handler slot `0x3F`), so it is read/write and the block should re-send to **restore**. There is **no arbitrary memory peek/poke** (the 8563's `ZRDWR` has no 859x equivalent; `MRD`/`MBRD`/`BRD` are Marker/Band reads, `OUTDAC` is a window-manager marker macro). Deep-RE cross-confirms hardware facts (A7-bus DACs `0xFFF700+2*idx` via `ACTDEF`/`ADCPBKT`; SCI ADC ports `0xFFF5FC/5FE` via `ADCSETUP`/`ADCXFR`). Read-only bench probe: [pkg/859x/probe859x.py](pkg/859x/probe859x.py); analysis: [pkg/859x/UNDOCUMENTED_COMMANDS.md](pkg/859x/UNDOCUMENTED_COMMANDS.md), [pkg/859x/GPIB_MEMORY_ACCESS.md](pkg/859x/GPIB_MEMORY_ACCESS.md).
...
**★ 2026-05-31 — the operating-loop freeze is CRACKED; the firmware now runs.** The 8593-strap analog-poll freeze (the `0x9A` poll above) is RESOLVED by the [analogbus.go](pkg/emu/device/analogbus.go) conversion model — the firmware now REACHES the operating loop. The new (and final) blocker was identified and solved: `fcn.18568` idle-waits in a loop at **ROM 0x188b6** that exits only when **`0xFFF300` bit 11 (sweep-complete)** is set (or a DLP/key queue has work). We modelled bit 12 (sweep-ready) but never bit 11, so the firmware idled forever. Asserting bit 11 un-sticks it → work path `0x18910` → continuous-sweep handler **`0x18a8c`** (re-arms via `bf34`/`f716`/`f70a`, processes via `fcn.171f6`, ACKs by writing `f300` at `0x1892A`). The two SKIPPED tests above + the "frozen at the poll" claim are now superseded. **State of the trace path** (M2): the **analog model is complete** ([pkg/emu/device/sweepengine.go](pkg/emu/device/sweepengine.go) wires `SpectrumModel`/`Detector`/`FrequencyModel` into the `0xFFF200` video-ADC — faithful, tunable, CAL peak + noise; `cmd/tracedemo` renders it on the UI). With the `f300` bit-11 sweep-clock, the **sweep cycle runs** (buffer fills → `fcn.171f6` processes → re-arm) and the **trace buffer at `0x2FD508` holds the correct spectrum** (verified: max=0x17F CAL peak, min=0x0 noise). The ONLY remaining gap is the trace **PAINT** — the `__GTTDRW` DLP command (`0x65986`) / source `0x5fa22` reading the buffer and drawing the line. Full trail: [docs/DRIVETICK_BLOCKER.md](docs/DRIVETICK_BLOCKER.md), [docs/TRACE_DISPLAY_PATH.md](docs/TRACE_DISPLAY_PATH.md). Tools added this session: `cmd/looptrace2` (idle-gate/sweep-clock), `cmd/tracehunt`, `cmd/rendertrace` (call-tree tracer via `CPU.RunUntil`), `cmd/gdbserver` (GDB remote stub — see below).

**★ 2026-06-04 — the sweep injection is now FAITHFUL; CAL DISP (and menus) no longer get wiped.** The GUI force-drove `DriveOneSweepChunk()` every frame, so `driveSweepCycle` injected phantom IRQ1/IRQ6 unconditionally (gated only on the `bf34` capture vector, which stays armed in cal mode). In CAL DISP mode that dragged the firmware through its sweep-redraw path — an **SCLR storm** that cleared the graph and wiped the cal table one frame after it drew (the cal display "appeared then was immediately replaced"). **Root cause** (proven headlessly, `pkg/emu/machine/caldisp_diag_test.go`, `DIAG=1`): with the forced sweep off, the firmware paints the full cal table and holds it; with it on, SCLR jumps 8× and the table is erased. **Fix**: `driveSweepCycle` now injects ONLY while the firmware is genuinely waiting for a sweep — detected by a `0xFFF300` (sweep-status) read from a **sweep-wait code region** (the measurement processor `fcn.171f6` ROM `0x171xx`, or the operating-loop idle-wait at ROM `0x188BA`). The spectrum display reaches those; CAL DISP / menus never do (they touch `0xFFF300` only via the sparse background poll at ROM `0x3F00`). Classified in [mmio.go](pkg/emu/device/mmio.go) (`PCFunc` + `ConsumeSweepRegionPoll`); the machine keeps a ~1000-chunk **hold** ([machine.go](pkg/emu/machine/machine.go) `sweepHold`) so injection free-runs between the (sparse, when fed) sweep-wait polls but decays within ~1 frame once the firmware leaves the sweep code. Empirical signal (cmd-trace PCs): `0x188BA` polled 75× (fed)/55393× (starved) in spectrum, **0× in cal**. Locked by `TestSweepRegionPollClassification` (device). **GUI graticule**: the periodically-repainted vector graticule isn't in the live buffer most frames, so `renderLive` showed it missing. The GUI now picks its render source per-frame from the display mode (`Machine.SweepIdle`): **stable snapshot while sweeping** (graticule grid + box + trace, flicker-free — the grid-line `maybeFrameSnapshot` trigger) and the **live buffer while idle** (CAL DISP / menus refresh; the sweep gate keeps that buffer stable now). Boot-screen golden regenerated to the complete spectrum render; `TestGraticuleGridVisible`/`TestTraceSolid` still pass.

**Annunciators + POST self-test (cracked).** The boot `FAIL: DF0F` is the power-on self-test result `NOT(0xFFF612):NOT(0xFFF610)` — A16 PASS-bit latches; 15/16 bits cleared with faithful HW models (POST-bypass strap `f614/f616`, `f700↔f780` loopback, `0x320000` write-address latch). The status annunciators (REF UNLOCK=A9 ref-lock, ADC-* FAIL=ADC self-test, OVEN COLD=oven timer) are a SEPARATE aggregation path (`fcn.11B9A` packs hardware-status flags `0xFFB1E0/B1F0/B1F6/B1F8/B1FA` into display words); verified code map (descriptor table `[0xFF9562]`: OVEN COLD=0x0D, ADC-TIME=0x28, FREQ UNCAL=0x23) and add/remove API (`fcn.e7f0`/`fcn.e87e`). They clear by modelling each subsystem's status good during boot. Full RE: [docs/POST_SELFTEST.md](docs/POST_SELFTEST.md). Tools: `cmd/failcode`, `cmd/post`, `cmd/anndesc`, `cmd/anncodemap`, `cmd/annbits`, `cmd/befprobe`.

**GDB remote debugger + `CPU.RunUntil`.** [pkg/emu/gdb](pkg/emu/gdb) + `cmd/gdbserver` expose the running emulator as a GDB remote target (registers, memory, breakpoints, single-step, read/write/access WATCHPOINTS via the bus `OnRead`/`OnWrite` hooks). Drive with `gdb -ex 'set architecture m68k' -ex 'target remote :3333'` (gef/pwndbg) or `rizin -a m68k -b 32 -d gdb://localhost:3333` (installed; register-role mapping is i386-centric). `CPU.RunUntil(cycles, stopPC)` (Musashi instruction-hook + `m68k_end_timeslice`) stops exactly at a target PC — the basis for `cmd/rendertrace`'s shadow-stack call-tree tracer that walks table-dispatched code the A6 chain can't. See [docs/DEBUGGING.md](docs/DEBUGGING.md).
...
### Real instrument (bench) — [pkg/859x/](pkg/859x/)

Ground-truth work on the actual **HP 8593E** (serial 155, firmware REV 940822, HP-IB address 7) over GPIB. **Status page + handoff: [pkg/859x/BENCH_STATUS.md](pkg/859x/BENCH_STATUS.md)** — read it before touching the instrument. Not part of the Go build.

- **Captured and versioned** in [pkg/859x/bench/data/](pkg/859x/bench/data/): the 446-value `CAL DUMP` correction-factor backup (three byte-identical reads, decoded in `data/cal/cal_constants_decoded.json`) and the full 2026-07-26 read-only sweep including **`27_diag_USTATE.txt` — the `USTATE?` user-memory block** (`#A`, length `0x147C`, 5244 B payload) that is the bench proof behind the memory-dump finding.
- **Two gotchas, each of which cost a session.** (1) The NI GPIB-USB-HS adapter comes up **not Controller-In-Charge** — it never asserts IFC, so *every* address 1–30 fails (`VI_ERROR_BERR` on Windows, `ENOL` on Linux), which misreads as "wrong address"; fix with `SendIFC(0)` / `ibrsc(1)`+`ibsic()` once per replug or power-cycle. (2) **The firmware silently ignores unknown commands** — `ERR?` stays `0,0,0,0` and the reply is whatever number is on the parser's value stack, so a plausible numeric answer proves nothing. Always interleave the impossible control command `ZQZXNOTACMD?`; both probe scripts do this automatically. This is *why* `docs/command_notes_empirical.json` overrides the disassembly notes.
- **Read-only by construction.** Never send `CAL INIT`/`CAL STORE`/`CAL FETCH`/`FACTSET`/`INIT FLAT`/`STORE FLATNESS`/`SET ATTN ERROR`; the scripts refuse them by denylist, and `CAL INIT` additionally needs CF = −37 Hz to arm, so leaving centre frequency alone keeps the destructive reinitialise blocked.
- **Scripts:** [pkg/859x/probe859x.py](pkg/859x/probe859x.py) (pyvisa read-only sweep, Windows/NI path — the one that produced the captures), [pkg/859x/gpibprobe_linux.py](pkg/859x/gpibprobe_linux.py) (drives `libgpib.so.0` through ctypes — needs only python3, for the Linux bench host which has no pyvisa/gpib bindings), [pkg/859x/bench/probe_cmd.py](pkg/859x/bench/probe_cmd.py) (is-this-command-real tester with the fake-command control).
- **`dump859x.py`/`dump.py` are dead ends, kept for their echo-guard only**: their `ZSETADDR`/`ZRDWR?` premise is an 8563 command with **no 859x equivalent** — this unit has no arbitrary memory peek. The real backup path is `CAL DUMP` + `USTATE?`; see [pkg/859x/GPIB_MEMORY_ACCESS.md](pkg/859x/GPIB_MEMORY_ACCESS.md).
- **Linux bench host (`yoda`) is not yet usable end-to-end.** The historical `0x15`/`-110` attach failure is **gone** (clean attach on linux-gpib 4.3.7 / kernel 6.8) and addressing works once IFC is pulsed, but `ibwrt` fails with `EDVR` from `ni_usb_gpib.c:816 NIUSB_ADDRESSING_ERROR` — the adapter firmware itself rejects the write, and the code is byte-identical to torvalds/master so a newer kernel cannot fix it. Remaining path is a USBPcap trace of NI-VISA's *write/addressing* sequence on Windows: [pkg/859x/LINUX_GPIB_NI_HS.md](pkg/859x/LINUX_GPIB_NI_HS.md), [pkg/859x/bench/LINUX_RESULTS.md](pkg/859x/bench/LINUX_RESULTS.md).
