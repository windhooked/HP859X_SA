...
| Enter key (PS/2 `0x5A`) | `cmd/hpibstep` PC-trace | Enter routes through `fcn.56d1a` (binary dispatcher) at code `0x9800` → nested jump table at `fcn.6862` → `0x56F2E` → mode-2 path at `0x56FB2` → `fcn.56e12` (ensures trailing `;`) → `fcn.56cd2` which **saves the buffer to a 40-slot HISTORY RING at 0xFFA634+**, NOT to an executor. Enter is "remember this line", not "run it". |
